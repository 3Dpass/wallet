...
const renderAddressContent = (address: string, metadata?: Record<string, React.ReactNode>, short: boolean = false) => {
  return (
  ...
  return (
    ...
    >
      <Button fill className="w-full justify-between items-center" rightIcon="caret-down">
        {selectedAddress ? (
          <div className="p-1">{renderAddressContent(selectedAddress, metadata, true)}</div>
        ) : (
