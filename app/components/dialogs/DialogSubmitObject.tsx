...
import { BufferGeometry, Mesh, Float32BufferAttribute, Vector3 } from "three";
...
export default function DialogSubmitObject({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handlePropChange = useCallback((idx: number, key: "propIdx" | "maxValue", value: number) => {
    setProperties((prev) => {
      return prev.map((p, i) => {
        if (i === idx) {
          if (key === "propIdx") {
            ...
            if (selectedProperty) {
              ...
              const isNonFungible = propertyName.includes('non-fungible') ||
  ...
  const addProperty = useCallback(() => {
    ...
    const availableProperty = propertyOptions.find(opt => !selectedPropIdxs.has(opt.value));
    ...
    setProperties([
      ...properties,
      { id: Date.now() + Math.random(), propIdx: availableProperty.value, maxValue: defaultMaxValue }
    ]);
  ...
  const handlePropertyValueChange = useCallback((idx: number, value: number) => {
    ...
    setProperties((prev) => {
      ...
      const selectedProperty = propertyOptions.find(opt => opt.value === property.propIdx);
      ...
      if (selectedProperty) {
        ...
        return prev.map((p, i) => i === idx ? { ...p, maxValue: validatedValue } : p);
      } else {
    ...
    async function fetchPropertyOptions() {
      ...
      try {
        ...
        entries.forEach(([storageKey, property]) => {
          ...
          const propIdx = parseInt(storageKey.args[0].toString(), 10);
          const prop = property.toJSON ? property.toJSON() : property;
          if (prop && typeof prop === 'object' && 'name' in prop && 'maxValue' in prop) {
  ...
  const filterNonEmptyHashes = useCallback((hash: Hash) => hash.value.trim() !== '', []);
  
  const filterValidProperties = useCallback((prop: Property) => 
    prop.propIdx >= 0 && prop.maxValue >= 0, []);
  
  const mapPropertyToApiFormat = useCallback((prop: Property) => ({
  ...
  }, [api, pair, objString, category, isPrivate, numApprovals, hashesEnabled, hashes, properties, toaster, t, handleSignAndSendCallback, filterNonEmptyHashes, filterValidProperties, mapPropertyToApiFormat]);
  ...
  const getSharePropertyOption = useCallback(() => {
    return propertyOptions.find(opt => opt.label.toLowerCase().includes('share'));
  }, [propertyOptions]);
  ...
  return (
    ...
    >
      <div className={Classes.DIALOG_BODY}>
        ...
        <FormGroup label={<span className="text-gray-500">Property rights:</span>}>
          <RadioGroup
            inline
        ...
        <FormGroup label={<span className="text-gray-500">OBJ File <i>(1 Mb max)</i>:</span>}>
            ...
            <div className="w-full h-48 mt-2 relative">
              {mesh?.geometry ? (
                ...
                <div className="flex flex-col items-center justify-center h-full p-4 text-center border border-gray-200 rounded">
                  <span className="text-gray-400 text-sm mb-1">Preview not available</span>
        ...
        <FormGroup label={<span className="text-gray-500">Approvals: <i>(N of confirmations of the object authenticity)</i></span>}>
        ...
        <FormGroup label={<span className="text-gray-500">Set the object properties:</span>}>
          {propertyOptionsLoading ? (
            ...
            <div className="space-y-2">
              {properties.map((p, i) => {
                ...
                const selectedPropIdxs = new Set(properties.filter((_, idx) => idx !== i).map(p => p.propIdx));
                const isShareProperty = p.propIdx === getSharePropertyOption()?.value;
                return (
                  <div key={`property-${p.id}`} className="flex gap-2 items-center">
                    <HTMLSelect
                      options={propertyOptions
                        .filter(opt => !selectedPropIdxs.has(opt.value) || opt.value === p.propIdx)
                    ...
                    <NumericInput 
                      min={0} 
                      max={selectedProperty ? (isNonFungible ? 1 : selectedProperty.maxValue) : undefined}
                      value={selectedProperty ? 
                        (isNonFungible ? 1 : Math.min(p.maxValue, selectedProperty.maxValue)) : 
                        p.maxValue
                      }
                    ...
                    />
                    <Button icon="cross" minimal onClick={createRemovePropertyCallback(i)} disabled={isShareProperty} />
                    {isNonFungible && (
                      <span className="text-sm text-gray-500">(Fixed at 1)</span>
                    )}
                    {isShare && (
                      <span className="text-sm text-gray-500 hidden sm:inline">(Auto-applied)</span>
                    )}
              ...
              <Button icon="plus" minimal onClick={addProperty} disabled={properties.length >= propertyOptions.length}>Add Property</Button>
            </div>
          )}
        </FormGroup>
      </div>
