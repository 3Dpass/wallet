...
export default function TokenHoldersPieChart({
  holders,
  total,
}: TokenHoldersPieChartProps) {
  if (!holders.length || total === 0) {
    return (
      <div className="text-center text-gray-500 py-4">No distribution data</div>
    );
  }
  ...
  const pieData = [
    ...top,
    ...(restTotal > 0 ? [{ accountId: "Other", balance: restTotal }] : []),
  ];
  // Pie chart geometry
  const size = 120;
  const radius = 54;
  ...
  return (
    <div className="flex flex-col items-center py-4">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        <title>Token Holders Distribution Pie Chart</title>
        {slices}
      </svg>
